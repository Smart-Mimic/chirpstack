...
impl Data {
    ...
    async fn handle_mac_commands(&mut self) -> Result<()> {
        ...
        if let lrwn::Payload::MACPayload(pl) = &self.phy_payload.payload {
            ...
            if pl.f_port == Some(198) || pl.f_port == Some(197) || pl.f_port == Some(186) || pl.f_port == Some(174) || pl.f_port == Some(158) {
                if let Some(lrwn::FRMPayload::Raw(ref payload_bytes)) = pl.frm_payload {
                    ...
                    if payload_hex.contains("00000000") {
                        info!(dev_eui = %self.device.as_ref().unwrap().dev_eui, "Port 197 with 00000000 pattern, sending DeviceTimeAns");
